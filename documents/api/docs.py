...
def _base_401_response(custom_message: str = None) -> OpenApiResponse:
    return OpenApiResponse(
        ...
        or (
            "Request’s credentials are missing or invalid. A valid JWT authentication"
            " is required."
        ),
    )
...
attachment_viewset_docs = {
    "retrieve": extend_schema(
        ...
        description=(
            "Permission to access the attachment is checked based on the containing"
            " document as follows:\n"
            "* Admin users are allowed access to the attachment if the containing"
            " document was stored using the service they are using and whose admins"
            " they are.\n"
            "* Authenticated users are allowed access to the attachment if they are the"
            " owner of the containing document."
            # TODO: Uncomment when organization features are implemented
            # " or the containing document is owned by an organization and the user has"
            # " permission to act on behalf of that organization.",
        ),
        responses={
            ...
            status.HTTP_403_FORBIDDEN: OpenApiResponse(
                description=(
                    "The authenticated user lacks the proper permissions to access the"
                    " document. Depending on the requested document, either the user"
                    " does not belong to the admin group of the service which owns the"
                    " document, the user does not own the document."
                    # TODO: Uncomment when organization features are implemented
            ...
            status.HTTP_404_NOT_FOUND: OpenApiResponse(
                description=(
                    "No document was found with `documentId` or the document did not"
                    " have an attachment `attachmentId`."
                )
            ),
    ...
    "create": extend_schema(
        ...
        description=(
            ...
            "* Authenticated users are allowed access to the document if they are the"
            " owner of the document.\n\n"
            # TODO: Uncomment when organization features are implemented
            # " or the document is owned by an organization and the user has permission"
            # " to act on behalf of that organization.\n\n"
            "The following rules apply:\n"
            "* Drafts may be modified by the owning user or the owning service's"
            " admin\n"
            # TODO: Uncomment when organization features are implemented
        ...
        responses={
            ...
            # status.HTTP_403_FORBIDDEN: OpenApiResponse(
            #     description=(
            #         "The request contains an organization ID and the currently"
            #         " authenticated user does not have permission to act on behalf of"
            #         " that organization."
            #     )
            # ),
            status.HTTP_500_INTERNAL_SERVER_ERROR: _base_500_response(),
    ...
    "destroy": extend_schema(
        ...
        description=(
            "Permission to remove the attachment is checked based on the containing"
            " document as follows:\n"
            "* Authenticated users are allowed to remove the attachment if they are the"
            " owner of the containing document.\n\n"
            # TODO: Uncomment when organization features are implemented
            # "or the containing document is owned by an organization and the user has"
            ...
            "* Attachments may not be removed if the containing document's"
            " `lockedAfter` date has passed. This should be done by removing the whole"
            " document."
        ),
        responses={
            ...
            status.HTTP_403_FORBIDDEN: OpenApiResponse(
                description=(
                    ...
                    " document. Depending on the requested document, either the user"
                    " does not belong to the admin group of the service which owns the"
                    " document or the user does not own the document."
                    # TODO: Uncomment when organization features are implemented
                    #   Replace the previous two lines with the following
                    # "either the user does not belong to the admin group of the"
                    # " service which owns the document, the user does not own the"
                    # " document or the user does not have permission to act on behalf"
                    # " of the organization which owns the document."
                )
            ),
            status.HTTP_404_NOT_FOUND: OpenApiResponse(
...
document_viewset_docs = {
    "list": extend_schema(
        ...
        description=(
            "This endpoint can be used to fetch a list of documents. "
            "The list can be filtered, sorted and paged using the appropriate query"
            " parameters.\n\n"
            "The results will contain only those documents which are allowed for the"
            " current user.\n\n"
            "* Admin users are allowed to fetch all documents which were stored using"
            " the service they are using and whose admins they are.\n"
            "* Authenticated users are allowed to fetch documents which are owned by"
            " them and which were stored using the service they are using."
            # TODO: Uncomment when organization features are implemented
            # "Documents owned by an organization are not returned even if such a"
            # " document was created by the current user.\n"
            # "* Authenticated users may fetch documents owned by an organization by"
            # " giving the organization's business ID in the search parameters. In this"
            # " case the user's permission to act on behalf of the organization is"
            # " verified and the results will contain only documents which are owned by"
        ...
        parameters=[
            ...
            OpenApiParameter(
                ...
                OpenApiTypes.STR,
                description=(
                    "Search for documents which are owned by the given business ID. "
                    "If this is given, the calling user must either be an admin"
                    # TODO: Uncomment when organization features are implemented
                    #   Replace the previous line with the following
                    # "If this is given, the calling user must either be an admin or "
                    # "have permission to act on behalf of the organization",
                ),
            ),
            OpenApiParameter(
                "user_id",
                OpenApiTypes.UUID,
                description=(
                    "Search for documents which are owned by the given user ID. "
            ...
            OpenApiParameter(
                ...
                OpenApiTypes.STR,
                description=(
                    "Search for documents with metadata matching key:value pairs"
                    " separated by comma. ( key:value, key:value ). Lookup method for"
                    " value is 'iexact', key must be exact and is case sensitive."
                ),
            ),
        ],
        responses={
    ...
    "retrieve": extend_schema(
        ...
        description=(
            ...
            "Permission to access the document is checked as follows:\n\n"
            "* Admin users are allowed access to the document if it was stored using"
            " the service they are using and whose admins they are.\n"
            "* Authenticated users are allowed access to the document if they are the"
            " owner of the document."
            # TODO: Uncomment when organization features are implemented
            #   Replace the previous line with the following
            # "* Authenticated users are allowed access to the document if they are the"
            # " owner of the document or the document is owned by an organization and"
    ...
    "batch_list": extend_schema(
        ...
        description=(
            "This endpoint allows a service to fetch multiple documents by their IDs"
            # TODO: Uncomment when organization features are implemented
            #   Replace the previous line with the following
            # "* Authenticated users are allowed access to the document if they are the"
            # " owner of the document or the document is owned by an organization and"
    ...
    "create": extend_schema(
        ...
        description=(
            ...
            "This endpoint supports API key authentication to enable unauthenticated"
            " users to store documents. Note that if this endpoint is used in such a"
            " way, accessing the stored documents later will only be possible using "
            "the appropriate admin credentials.\n\n"
            "This endpoint also supports authentication using a Bearer token, similarly"
            " to the other endpoints. In this case the authenticated user is able to"
        ...
        responses={
            ...
            #     description=(
            #         "The request contains an organization ID and the currently"
            #         " authenticated user does not have permission to act on behalf"
            #         " of that organization."
            #     )
            # ),
            status.HTTP_500_INTERNAL_SERVER_ERROR: _base_500_response(),
    ...
    "destroy": extend_schema(
        ...
        description=(
            ...
            # TODO: Uncomment when organization features are implemented
            # "or an organization's representative, "
            # "if the document is owned by an organization. This is possible even if"
            # " the `lockedAfter` date has passed, to enable a user to remove expired"
            # " applications."
        ),
        responses={
            status.HTTP_204_NO_CONTENT: OpenApiResponse(
                description=(
                    "The specified Document and its attachments were removed"
                    " successfully"
                ),
            ),
            (status.HTTP_400_BAD_REQUEST, "application/json"): _base_400_response(),
...
document_metadata_viewset_docs = {
    "retrieve": extend_schema(
        ...
        description=(
            "List users documents parts that doesn't contain sensitive information to"
            " easily see current applications and documents of a single user across"
            " services."
        ),
        # "of that organization.",
        responses={
            ...
            status.HTTP_403_FORBIDDEN: OpenApiResponse(
                description=(
                    "Current authentication doesn't allow viewing of this users"
                    " documents"
                )
            ),
            status.HTTP_404_NOT_FOUND: OpenApiResponse(
...
document_gdpr_viewset = {
    "retrieve": extend_schema(
        ...
        summary=(
            "List user's document details and number of deletable and undeletable"
            " documents."
        ),
        responses={
            (status.HTTP_200_OK, "application/json"): OpenApiResponse(
    ...
    "destroy": extend_schema(
        examples=[example_gdpr_api_repsonse],
        description=(
            "Delete user's documents from a single service that aren't under"
            " contractual obligation."
        ),
        summary=(
            "List user's documents that weren't deleted. Deletable field should be"
            " zero."
        ),
        responses={
            (status.HTTP_200_OK, "application/json"): OpenApiResponse(
                ...
                description=(
                    ...
                    " been removed. Documents with contractual obligations are returned"
...
document_statistics_viewset_docs = {
    "list": extend_schema(
        ...
        description=(
            "Lists non sensitive data of all documents in ATV. Service staff can fetch"
            " data from the respective service. Currently the use case is to verify"
        ...
        responses={
            ...
            status.HTTP_403_FORBIDDEN: OpenApiResponse(
                description=(
                    "Current authentication doesn't allow viewing document statistics"
                )
            ),
            status.HTTP_500_INTERNAL_SERVER_ERROR: _base_500_response(),
        },
        ...
        parameters=[
            OpenApiParameter(
                ...
                OpenApiTypes.STR,
                description=(
                    "Search for documents with the given statuses separated by comma."
                ),
            ),
            OpenApiParameter(
                "types",
                OpenApiTypes.STR,
                description=(
                    "Search for documents with the given types separated by comma"
                ),
            ),
            OpenApiParameter(
                "services",
                OpenApiTypes.STR,
                description=(
                    "Search for documents with the given services separated by comma"
                ),
            ),
            OpenApiParameter(
                "transaction_id",
...
document_status_history_viewset_docs = {
    ...
    "create": extend_schema(
        ...
        description=(
            "Allows creating either individual new status, activity or both status and"
        ...
        responses={
            ...
            (status.HTTP_200_OK, "application/json"): OpenApiResponse(
                ...
                description=(
                    ...
                    " formated. For example if same status is posted twice. Returns"
