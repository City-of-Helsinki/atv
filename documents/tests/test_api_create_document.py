...
def test_create_document_invalid_fields(service_api_client):  # , snapshot):
...
def test_create_document_many_files(api_client, service_api_client, snapshot):
    data = {
        "attachments": [
            SimpleUploadedFile(
...
def test_create_document_file_limit(api_client, service_api_client, snapshot):
    data = {
        **VALID_DOCUMENT_DATA,
        "attachments": [
            SimpleUploadedFile(
    ...
    body = response.json()
    assert body == {"errors": ["Cannot upload files larger than 20MB"]}
